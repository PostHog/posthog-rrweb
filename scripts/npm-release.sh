...
for PACKAGE in "${PACKAGES[@]}"; do
  ...
  pnpm --filter "$PACKAGE" publish --dry-run --access public --no-git-checks || { echo "Failed to dry-run publish $PACKAGE"; exit 1; }
done

# then for real
for PACKAGE in "${PACKAGES[@]}"; do
  echo "Publishing $PACKAGE"
  pnpm --filter "$PACKAGE" publish --access public --no-git-checks || { echo "Failed to publish $PACKAGE"; exit 1; }
done
